...
@property (weak, nonatomic) IBOutlet UIButton *obstacleAvoidanceButton;
@property (weak, nonatomic) IBOutlet UIButton *collisionAvoidanceButton;
@property (weak, nonatomic) IBOutlet UIButton *pilotAssistanceButton;
...
-(void)viewWillAppear:(BOOL)animated {
    ...
    [self updateFlightControllerState];
}
...
- (IBAction)toggleObstacleAvoidance:(id)sender {
    DJIFlightAssistant *flightAssistant = [[DemoUtility fetchFlightController] flightAssistant];
    UIAlertAction *cancelAction = [UIAlertAction actionWithTitle:@"OK" style:UIAlertActionStyleCancel handler:nil];

    WeakRef(target);
    [flightAssistant getActiveObstacleAvoidanceEnabledWithCompletion:^(BOOL enabled, NSError * _Nullable error) {
        if (enabled) {
            [flightAssistant setActiveObstacleAvoidanceEnabled:false withCompletion:^(NSError * _Nullable error) {
                WeakReturn(target)
                if (error) {
                    NSString *message = [NSString stringWithFormat:@"Failed to disable obstacle avoidance %@", error.localizedDescription];
                    [DemoUtility showAlertViewWithTitle:nil message:message cancelAlertAction:cancelAction defaultAlertAction:nil viewController:target];
                }

                [target updateFlightControllerState];
            }];
        } else {
            [flightAssistant setActiveObstacleAvoidanceEnabled:true withCompletion:^(NSError * _Nullable error) {
                WeakReturn(target)
                if (error) {
                    NSString *message = [NSString stringWithFormat:@"Failed to enable obstacle avoidance %@", error.localizedDescription];
                    [DemoUtility showAlertViewWithTitle:nil message:message cancelAlertAction:cancelAction defaultAlertAction:nil viewController:target];
                }
...
- (IBAction)toggleCollisionAvoidance:(id)sender {
    DJIFlightAssistant *flightAssistant = [[DemoUtility fetchFlightController] flightAssistant];
    UIAlertAction *cancelAction = [UIAlertAction actionWithTitle:@"OK" style:UIAlertActionStyleCancel handler:nil];

    WeakRef(target);
    [flightAssistant getCollisionAvoidanceEnabledWithCompletion:^(BOOL enabled, NSError * _Nullable error) {
        if (enabled) {
            [flightAssistant setCollisionAvoidanceEnabled:false withCompletion:^(NSError * _Nullable error) {
                WeakReturn(target)
                if (error) {
                    NSString *message = [NSString stringWithFormat:@"Failed to disable collision avoidance %@", error.localizedDescription];
                    [DemoUtility showAlertViewWithTitle:nil message:message cancelAlertAction:cancelAction defaultAlertAction:nil viewController:target];
                }
            ...
            [flightAssistant setUpwardsAvoidanceEnabled:false withCompletion:^(NSError * _Nullable error) {
                WeakReturn(target);
                if (error) {
                    NSString *message = [NSString stringWithFormat:@"Failed to disable upwards avoidance %@", error.localizedDescription];
                    [DemoUtility showAlertViewWithTitle:nil message:message cancelAlertAction:cancelAction defaultAlertAction:nil viewController:target];
                }
            ...
            [flightAssistant setCollisionAvoidanceEnabled:true withCompletion:^(NSError * _Nullable error) {
                WeakReturn(target);
                if (error) {
                    NSString *message = [NSString stringWithFormat:@"Failed to enable collision avoidance %@", error.localizedDescription];
                    [DemoUtility showAlertViewWithTitle:nil message:message cancelAlertAction:cancelAction defaultAlertAction:nil viewController:target];
                }
            ...
            [flightAssistant setUpwardsAvoidanceEnabled:true withCompletion:^(NSError * _Nullable error) {
                WeakReturn(target);
                if (error) {
                    NSString *message = [NSString stringWithFormat:@"Failed to enable upwards avoidance %@", error.localizedDescription];
                    [DemoUtility showAlertViewWithTitle:nil message:message cancelAlertAction:cancelAction defaultAlertAction:nil viewController:target];
                }
...
- (IBAction)togglePilotAssistance:(id)sender {
    DJIFlightAssistant *flightAssistant = [[DemoUtility fetchFlightController] flightAssistant];
    UIAlertAction *cancelAction = [UIAlertAction actionWithTitle:@"OK" style:UIAlertActionStyleCancel handler:nil];

    WeakRef(target);
    [flightAssistant getAdvancedPilotAssistanceSystemEnabledWithCompletion:^(BOOL enabled, NSError * _Nullable error) {
        if (enabled) {
            [flightAssistant setAdvancedPilotAssistanceSystemEnabled:false withCompletion:^(NSError * _Nullable error) {
                WeakReturn(target);
                if (error) {
                    NSString *message = [NSString stringWithFormat:@"Failed to disable pilot assistance %@", error.localizedDescription];
                    [DemoUtility showAlertViewWithTitle:nil message:message cancelAlertAction:cancelAction defaultAlertAction:nil viewController:target];
                }
            ...
            [flightAssistant setAdvancedPilotAssistanceSystemEnabled:true withCompletion:^(NSError * _Nullable error) {
                WeakReturn(target);
                if (error) {
                    NSString *message = [NSString stringWithFormat:@"Failed to enable pilot assistance %@", error.localizedDescription];
                    [DemoUtility showAlertViewWithTitle:nil message:message cancelAlertAction:cancelAction defaultAlertAction:nil viewController:target];
                }
...
- (void)updateFlightControllerState {
    DJIFlightController *fc = [DemoUtility fetchFlightController];

    UIButton *obstacleAvoidanceButton = self.obstacleAvoidanceButton;
    UIButton *collisionAvoidanceButton = self.collisionAvoidanceButton;
    UIButton *pilotAssistanceButton = self.pilotAssistanceButton;

    [fc.flightAssistant getActiveObstacleAvoidanceEnabledWithCompletion:^(BOOL enabled, NSError * _Nullable error) {
        NSString *title = [NSString stringWithFormat:@"Obstacle Avoidance %s", enabled ? "On":"Off"];
        [obstacleAvoidanceButton setTitle:title forState:UIControlStateNormal];
    }];

    [fc.flightAssistant getCollisionAvoidanceEnabledWithCompletion:^(BOOL enable, NSError * _Nullable error) {
        NSString *title = [NSString stringWithFormat:@"Collision Avoidance %s", enable ? "On":"Off"];
        [collisionAvoidanceButton setTitle:title forState:UIControlStateNormal];
    }];

    [fc.flightAssistant getAdvancedPilotAssistanceSystemEnabledWithCompletion:^(BOOL enabled, NSError * _Nullable error) {
        NSString *title = [NSString stringWithFormat:@"Pilot Assistance %s", enabled ? "On":"Off"];
        [pilotAssistanceButton setTitle:title forState:UIControlStateNormal];
    }];
}
